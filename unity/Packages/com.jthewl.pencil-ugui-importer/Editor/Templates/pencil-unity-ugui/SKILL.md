...
2. Run `openpencil-ugui doctor` from the Unity project root.
3. Use the configured canvas provider reference under `references/<provider>/`.
4. Create or update the provider design file.
5. Export UI IR JSON with `openpencil-ugui export`.
6. Import UI IR into Unity with `openpencil-ugui import` while a Canvas is selected in the Hierarchy.
